...
{
    // UART deaktivieren (Receiver und Transmitter aus)
    UCSR1B &= ~((1 << RXEN1) | (1 << TXEN1));
    
    // Optionale Rücksetzung der Control-Register
    UCSR1A = 0x00;
    UCSR1B = 0x00;
    UCSR1C = 0x00;

    // Baudrate-Register zurücksetzen
    UBRR1H = 0x00;
    UBRR1L = 0x00;

    // Pins für RXD1 (PD2) und TXD1 (PD3) als GPIO konfigurieren
    // Setze beide Pins als Eingänge und keine Pullups aktiv
    DDRD &= ~((1 << PD2) | (1 << PD3));  // Setze PD2 und PD3 als Eingänge
    
    // Optional: Pullups deaktivieren, falls aktiviert waren
    PORTD &= ~((1 << PD2) | (1 << PD3)); // Setze die Pins auf Low (keine Pullups)
    
    // Alternativ: Pins als Ausgänge setzen
    // DDRD |= (1 << PD2) | (1 << PD3);    // Setze PD2 und PD3 als Ausgänge
    // PORTD &= ~((1 << PD2) | (1 << PD3)); // Setze Low-Level auf die Pins (für Ausgänge)
}
